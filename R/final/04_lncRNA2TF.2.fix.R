...
diff<- pf.get.diff()
diff.5343<- pf.get.diff(c('lncRNA','pcg'))
...
cs4.abs2<-pf.multicor(abs(a4),b,rds='./data/lnc2actf.cs5946.cor.4.abs.2.rds',method = 'spearman')
...
tfs0['TP53']
#####
lncRNA2AR<-filter(lnctf.cor.indirect,v2=='AR')
############################
